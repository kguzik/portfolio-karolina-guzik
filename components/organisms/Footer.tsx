    ...
    <footer className={styles.footer}>
      <Container>
          ...
          <div className={styles.contact}>
            {contactDetails.map((detail, index) => (
              <div key={index} className='flex gap-3 items-center'>
                <detail.icon size={20} className='text-violet' />
                {detail.type === 'phone' ? (
                  <a href={`tel:${detail.text}`} className={styles.detailLink}>
                    {detail.text}
                  </a>
                ) : detail.type === 'email' ? (
                  <a
        ...
        <p className={styles.copyright}>
          Copyright © {new Date().getFullYear()} Karolina Guzik All rights
          reserved.
        </p>
      </Container>
    </footer>
...
const styles = {
  ...
  socials: 'flex gap-3',
  social:
    'w-10 h-10 bg-violet text-white flex items-center justify-center transition-colors rounded hover:bg-violet-dark',
